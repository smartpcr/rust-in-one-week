...
use windows::Win32::Networking::Clustering::{
    ...
    OpenClusterGroupW, CLUSTER_GROUP_STATE, HGROUP, HNODE,
};
...
impl Group {
    ...
    pub fn open(cluster: &Cluster, group_name: &str) -> Result<Self> {
        ...
        let handle = unsafe { OpenClusterGroupW(cluster.handle(), PCWSTR(wide_name.as_ptr())) };
    ...
    pub fn state(&self) -> Result<(GroupState, Option<String>)> {
        ...
        let state =
            unsafe { GetClusterGroupState(self.handle, PCWSTR(ptr::null_mut()), &mut size) };
        ...
        let state =
            unsafe { GetClusterGroupState(self.handle, PCWSTR(buffer.as_mut_ptr()), &mut size) };
    ...
    pub fn online(&self) -> Result<()> {
        let result = unsafe { OnlineClusterGroup(self.handle, HNODE::default()) };
    ...
    pub fn move_to(&self, node: &Node) -> Result<()> {
        let result = unsafe { MoveClusterGroup(self.handle, node.handle) };
        // ERROR_IO_PENDING (997) is acceptable - means operation is in progress
        if result != 0 && result != 997 {
            return Err(ClusError::WindowsError(WinError::from_win32()));
        }
