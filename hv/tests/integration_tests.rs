...
//! On non-Windows platforms, these tests are skipped automatically.
//!
//! Test organization:
//! - Unit tests: Run with `cargo test` (always enabled)
//! - Integration tests: Run with `cargo test --features integration` (requires Hyper-V)

#[cfg(all(windows, feature = "integration"))]
mod integration {
    //! Integration tests that require Hyper-V to be installed and running.
    //! Enable with: cargo test --features integration

    use hv::{HvError, HyperV, SnapshotType, SwitchType, VmGeneration, VmState};
...
mod unit {
    ...
    //! These tests run with regular `cargo test`.

    use hv::{HvError, SwitchType, VhdFormat, VhdType, VmState};
