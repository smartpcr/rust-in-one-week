...
use crate::seesaw::traits::{Comparable, CompResult};
...
fn solve_group(g: &Group, prev_measurements: i8, steps: Vec<String>) -> Result<FindPersonResult, String> {
    if g.people.len() == 1 {
        ...
        if rg3.found_group {
            let g = group_of3.iter().find(|&x| x.name == rg3.group_name).unwrap();
